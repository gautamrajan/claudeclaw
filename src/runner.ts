...
async function runClaudeOnce(
  ...
  const proc = Bun.spawn(args, {
    stdout: "pipe",
    stderr: "pipe",
    env: buildChildEnv(baseEnv, model, api, opts),
  });
...
async function execClaude(name: string, prompt: string, threadId?: string, unattended: boolean = true): Promise<RunResult> {
  ...
  let exec = await runClaudeOnce(args, primaryConfig.model, primaryConfig.api, baseEnv, timeoutMs, { unattended });
  ...
  if (primaryRateLimit && hasModelConfig(fallbackConfig) && !sameModelConfig(primaryConfig, fallbackConfig)) {
    ...
    exec = await runClaudeOnce(args, fallbackConfig.model, fallbackConfig.api, baseEnv, timeoutMs, { unattended });
    usedFallback = true;
  }
  ...
  if (COMPACT_TIMEOUT_ENABLED && exitCode === 124 && !isNew && existing) {
    ...
    if (compactOk) {
      ...
      const retryExec = await runClaudeOnce(args, primaryConfig.model, primaryConfig.api, baseEnv, timeoutMs, { unattended });
...
async function streamClaude(
  ...
  const childEnv = buildChildEnv(cleanEnv as Record<string, string>, model, api, { unattended: false });
